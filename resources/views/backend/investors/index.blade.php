                                        ...
                                        <tbody id="tbl-body-investor">
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <input name="status" type="checkbox" id="switch-blue" class="switch inp-status btn-change" checked="checked" value="true" data-url="{{ route('AdminInvestorChangeStatus', $investor->id) }}"/>
                                                    @else
                                                    <input name="status" type="checkbox" id="switch-blue" class="switch inp-status btn-change" value="true" data-url="{{ route('AdminInvestorChangeStatus', $investor->id) }}"/>
                                                    @endif
                                                    </div>
                                                </td>
                                                <td class="text-center">
                                                	...
                                                	<button class="btn btn-danger btn-delete-investor" data-url="{{ route('AdminInvestorGetDelete',$investor->id) }}" title="Xóa"><i class="fas fa-trash"></i></button>
                                                </td>
                                            </tr>
                                            @endforeach
        ...
        <div class="modal fade" id="add-investor" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
                    ...
                    <form action="{{ route('AdminInvestorsPostAdd') }}" id="frm-add-investor" enctype="multipart/form-data">
						<div class="modal-body">
							<div class="row">
								...
								<div class="form-group-investor">
									<div class="button-switch">
                                        <input name="status" type="checkbox" id="switch-blue" class="switch inp-status" value="true" />
                                    </div>
								</div>
...
jQuery(function($) {
            ...
            if(data.status ==1){
                $('#tbl-body-investor').prepend(`
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <div class="button-switch">
                                                        <input name="status" type="checkbox" id="switch-blue" class="switch inp-status btn-change" checked="checked" value="true" data-url="${data.url_status}"/>
                                                    </div>
                                                </td>
                                                <td class="text-center">
                                                    <button class="btn btn-primary edit-investor" title="Chỉnh sủa" data-url="${data.url_update}">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <button class="btn btn-danger btn-delete-investor" data-url="${data.url_delete}" title="Xóa"><i class="fas fa-trash"></i></button>
                                                </td>
                                            </tr>
                `);
            }else{
                $('#tbl-body-investor').prepend(`
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <div class="button-switch">
                                                        <input name="status" type="checkbox" id="switch-blue" class="switch inp-status btn-change" value="true" data-url="${data.url_status}"/>
                                                    </div>
                                                </td>
                                                <td class="text-center">
                                                    <button class="btn btn-primary edit-investor" title="Chỉnh sủa" data-url="${data.url_update}">
    ...
    $('#tbl-body-investor').on('click','.btn-delete-investor', function(){
        ...
        if(confirm('Bạn chắc chắn muốn xóa!')){
            axios.get(url).then(response => {
                ...
                $.notify('Không thể xóa chủ đầu tư', 'error');
            });
        }
    });
    ...
    $('#tbl-body-investor').on('click', '.edit-investor', function(e){
        ...
        axios.get(url, data).then(response => {
            ...
            $.notify('Không tìm thấy thông tin chủ đầu tư cần chỉnh sửa', 'error');
        });
    });
